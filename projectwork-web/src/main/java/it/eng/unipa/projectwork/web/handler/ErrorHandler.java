...
public class ErrorHandler implements javax.ws.rs.ext.ExceptionMapper<Throwable> {
	public javax.ws.rs.core.Response toResponse(Throwable exn) {
		...
		if (getCause(EJBAccessException.class, exn) != null) {
			return Response.status(403).type("application/json")
					.entity(new Message("User profile is not authorized\"]}")).build();
			...
			if (cause != null) {
				return Response.status(400).type("application/json").entity(new Message(cause.getErrorMessages()))
						.build();
			} else {
